...
struct ThemeCatalogTests {
    ...
    @Test func discoversThemeFiles() throws {
    ...
    @Test func themeContentReadsCorrectly() throws {
        let dir = try makeTempThemesDir()
        ...
        #expect(content != nil)
        #expect(try #require(content?.contains("background = #1e1e2e")))
    }

    @Test func missingThemeReturnsNil() throws {
        let dir = try makeTempThemesDir()
        let catalog = ThemeCatalog(themesDirectory: dir)
        #expect(catalog.themeContent(for: "Nonexistent Theme") == nil)
    }

    @Test func excludesDotFiles() throws {
        let dir = try makeTempThemesDir()
    ...
    @Test func loadPreviewsPopulatesCache() throws {
        let dir = try makeTempThemesDir()
    ...
    @Test func loadPreviewsCachesResults() throws {
        let dir = try makeTempThemesDir()
    ...
    @Test func loadPreviewsSkipsInvalidFiles() throws {
        let dir = try makeTempThemesDir()
    ...
    @Test func previewIsLightFlag() throws {
        let dir = try makeTempThemesDir()
    ...
    @Test func previewHas16PaletteColors() throws {
        let dir = try makeTempThemesDir()
