...
struct OpsxView: View {
    ...
    private var emptyState: some View {
        VStack(spacing: Spacing.lg) {
            ...
            Text("No active changes")
                ...
                .foregroundStyle(Color(tokens.textMuted))
            Text("Run `openspec init` in a repo to get started with OPSX")
                .font(.system(size: TypeScale.captionSize))
            ...
            ForEach(entry.changes) { change in
                ...
                OpsxChangeCard(
                    ...
                    onReadArtifact: { artifactID in
                        appModel.opsxModel.readArtifactContent(
    ...
    private var allRepoRoots: [String] {
        Array(appModel.opsxModel.statusByRepo.keys).sorted()
    }
    ...
    private func filteredEntries(repos: [String]) -> [RepoEntry] {
        ...
        return filtered.compactMap { repo in
            let status = appModel.opsxModel.status(for: repo)
...
private struct RepoEntry {
    let repoRoot: String
    let changes: [OpsxChange]
}

// MARK: - Change Card

private struct OpsxChangeCard: View {
    let change: OpsxChange
    let tokens: DesignTokens
    let isExpanded: Bool
    let onToggleExpand: () -> Void
    let onReadArtifact: (OpsxArtifactID) -> String?

    @State private var hoveredArtifactID: OpsxArtifactID?
    @State private var selectedArtifact: SelectedArtifact?

    private struct SelectedArtifact: Equatable {
        let id: OpsxArtifactID
        let content: String
    }
    ...
    private func artifactColor(state: OpsxArtifactState) -> Color {
        switch state {
    ...
    private func artifactNode(_ artifact: OpsxArtifact) -> some View {
    ...
    private func artifactStateIcon(_ state: OpsxArtifactState) -> some View {
        Group {
            switch state {
            case .done:
    ...
    private func nodeBackground(state: OpsxArtifactState, isHovered: Bool, isSelected: Bool) -> Color {
        if isSelected {
