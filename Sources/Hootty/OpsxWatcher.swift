...
/// Watches `openspec/` directories for changes using DispatchSource.
/// Monitors `openspec/`, `openspec/changes/`, and `openspec/archive/` subdirectories
/// to detect new/removed/archived changes. One watcher per repo root.
@MainActor
final class OpsxWatcher {
    ...
    /// Set the callback invoked when any watched `openspec/` directory changes.
    /// The parameter is the repo root path.
    func setOnChange(_ handler: @escaping (String) -> Void) {
        onChange = handler
    }

    /// Start watching the `openspec/` directory at the given repo root.
    /// Does nothing if already watching this root.
    func startWatching(repoRoot: String) {
        guard entries[repoRoot] == nil else { return }

        let opsxDir = (repoRoot as NSString).appendingPathComponent(OpsxModel.directoryPath)
        let changesDir = (opsxDir as NSString).appendingPathComponent("changes")
        let archiveDir = (opsxDir as NSString).appendingPathComponent("archive")

        var entry = WatchEntry()
        for dir in [opsxDir, changesDir, archiveDir] {
