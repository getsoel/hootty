...
`Array.remove(atOffsets:)` is a SwiftUI extension, not Foundation. In UI-free targets (PrompttyCore), use manual reverse iteration: `for index in offsets.reversed() { array.remove(at: index) }`.
...
`PrompttyCore.Tab` collides with `SwiftUI.Tab` (macOS 15+). In view code, qualify as `PrompttyCore.Tab` in function signatures to disambiguate.
